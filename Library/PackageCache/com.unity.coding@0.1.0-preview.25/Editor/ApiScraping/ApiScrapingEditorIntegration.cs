...
namespace Unity.Coding.Editor.ApiScraping
{
    class ApiScrapingSingleton : ScriptableSingleton<ApiScrapingSingleton>
    {
        ...
        //for tests. If someone overrides scrape mode during this process, we enable the scraper in test runs
    ...
    static class ApiScrapingEditorIntegration
    {
        ...
        static bool s_ScrapeIfNotEntry;
        static ApiScrapingEditorIntegration()
        {
            s_Process = new ApiScraperProcess();
            s_Scraper = new ApiScraperManager(s_Process);

            if (!ApiScrapingSingleton.instance.isScrapeModeOverridden && (Environment.GetCommandLineArgs().Contains("-runTests") || Environment.GetCommandLineArgs().Contains("-runEditorTests")))
            {
        ...
        public static void ForceScanAndScrape()
        {
            var oldScrapeMode = s_Scraper.scrapeMode;
            s_Scraper.scrapeMode = ScrapeMode.EnsureSuccess;
            var temp = s_ScrapeIfNotEntry;
            s_ScrapeIfNotEntry = true;
            ScanAndScrape();
            s_ScrapeIfNotEntry = temp;
            s_Scraper.scrapeMode = oldScrapeMode;
        }

        public static void ScanAndScrape()
        {
            ...
            if (scrapedDlls.Count == 0)
                Console.WriteLine("Updated 0 .api files.");
            else
                Console.WriteLine($"Updated {scrapedDlls.Count} .api files:\n{string.Join("\n", scrapedDlls.Select(c => Path.GetFileName(c.AssemblyFullPath) ))}");
        }
        ...
        static ScraperEntry ScrapeIfConfigured(string path)
        {
            ...
            if (asmdefPath != null && IsConfiguredToScrape(asmdefPath))
            {
                ...
                //scrapeFilePath needs the full path to allow scraping of local packages hosted outside /Packages or /Assets.
                var scrapeFilePath = Path.GetFullPath(Path.Combine(Path.GetDirectoryName(asmdefPath), Path.GetFileNameWithoutExtension(path) + ".api"));
        ...
        {
            string fullAsmdefPath = Path.GetFullPath(asmdefPath);
            using (s_IsChildOfProjectMarker.Auto())
            {
#if UNITY_2023_1_OR_NEWER
                if (Utilities.IsFolderImmutable(asmdefPath))
                    return false;
#else
                if (CodingToolsInternal.Bridge.AssetDatabaseInternal.IsFolderImmutable(asmdefPath))
                    return false;
#endif
            }
